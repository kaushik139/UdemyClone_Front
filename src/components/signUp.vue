<template>
  <v-sheet width="500" class="mx-auto p-3" style="text-align: center;margin-top: -30px;">
    ...
    <v-form fast-fail @submit.prevent>
      <div :class="[contain]">
        <h3 class="headdingPurple">Register Now <span :style="{color: symbol}">!</span></h3>
      ...
      <v-btn type="submit" block class="mt-2" :class="[bttn]" @click="submit">Submit</v-btn>
...
export default {
  ...
  data: () => ({
    ...
    rules: {
      ...
      valid: (v) => /^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{8,}$/.test(v) || "Password must contain minimum eight characters, at least one letter and one number."
    },
  }),
  ...
  methods: {
    async submit() {
      ...
      if (
       ...
       await axios.post(`http://localhost:3000/${this.getUser}/newSignup`, {
          name: this.name,
          email: this.email,
          password: this.password
        })
